...
<head>
    ...
    <title>You've Been Nominated</title>
    @php
    ...
    <div class="wrapper">
        <div class="card">
            <div class="header">
                ...
                <p class="subtitle">Your nomination to join the AHHC care team is underway.</p>
            </div>
            <div class="body">
                <p>Hello <strong>{{ $nomination->worker_full_name }}</strong>,</p>
                <p>You have been nominated to join <strong style="color:#19B0A5;">{{ $organization ?? config('app.name', 'AHHC Portal') }}</strong> as a care worker or service provider.</p>
                <div class="panel">
                    <div class="panel-inner">
                        <p class="panel-heading">Your assignment</p>
                        <table class="panel-table">
                            <tr><td class="label">Participant</td><td>{{ $nomination->participant->first_name }} {{ $nomination->participant->last_name }}</td></tr>
                            <tr><td class="label">Service Type</td><td>{{ $nomination->service_type }}</td></tr>
                            <tr><td class="label">Proposed Start Date</td><td>{{ $nomination->start_date?->format('d M Y') ?? 'TBA' }}</td></tr>
                        </table>
                    </div>
                </div>
                <div class="support-panel">
                    <div class="panel-inner">
                        ...
                        <p>- You cannot access the portal until your onboarding invitation is sent.<br>- Your access will be managed by AHHC administrators.<br>- Confidential participant data is not available until onboarding is complete.</p>
                    </div>
                </div>
                <div class="button-wrap">
                    <a class="button" href="{{ route('public.home') }}">Visit AHHC Portal Website</a>
                </div>
